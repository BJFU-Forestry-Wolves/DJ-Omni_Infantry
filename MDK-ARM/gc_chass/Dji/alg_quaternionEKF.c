 ...
 *  LastEditTime : 2023-02-10 15:48:00
 */
...
float QuaternionEKF_K[18];
float QuaternionEKF_H[18];
QEKF_INS_t QEKF_INS;
...
void QuaternionEKF_Init(float process_noise1, float process_noise2, float measure_noise, float lambda, float lpf) {
    ...
    QEKF_INS.ChiSquareTestThreshold = 1e-8;//卡方因子，
...
static void QuaternionEKF_Observe(Kalman_KalmanTypeDef *kf) {
    ...
    memcpy(QuaternionEKF_K, kf->K_data, sizeof(QuaternionEKF_K));
    memcpy(QuaternionEKF_H, kf->H_data, sizeof(QuaternionEKF_H));
}
